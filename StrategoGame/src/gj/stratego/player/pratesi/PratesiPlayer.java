...
public class PratesiPlayer implements Player {
	public static boolean isFirst;
	public static final int[][] PIECE_POS_FIRST = { { 0, 0 }, { 1, 0 }, { 0, 1 }, { 0, 2 }, { 2, 1 }, { 2, 3 },
			{ 1, 4 }, { 2, 0 }, { 0, 3 }, { 2, 2 } };
	public static final int[][] PIECE_POS_SECOND = { { 9, 0 }, { 6, 0 }, { 8, 0 }, { 9, 2 }, { 7, 1 }, { 7, 3 },
			{ 8, 4 }, { 7, 0 }, { 9, 3 }, { 7, 2 } };

	public final static int[][] FORBIDDEN_CELLS = { { 4, 2 }, { 4, 3 }, { 5, 2 }, { 5, 3 }, { 4, 6 }, { 4, 7 },
			{ 5, 6 }, { 5, 7 } };
	...
	public static int[][] oppYZ = new int[10][2];

	public PratesiPlayer() {
	...
	public void start(boolean isFirst) {
		PratesiPlayer.isFirst = isMyTurn = isFirst;
	...
	public int[] position(String piece) {
		...
		return isFirst ? PIECE_POS_FIRST[i] : PIECE_POS_SECOND[i];
	}
	...
	private static int[] getMyMove(int[] myMove) {
	...
	public void viewPositions(int[][] position) {
		...
		for (; i < end; i++) {
			...
			for (int op = 0; op < a.length; op++) {
				if (a[op] == 1) {
					board[i][op] = "OPP";
					oppYZ [r++] = new int[] { i, op };
				}
			}
		}
		startBoard();
		//StrategoPanel.setOppPosBoard(temp);
	}
	...
	private Position findFirstFreeCell(int[] current, int index) {
		...
		return neighbours.stream()
				.filter(e -> isNull(e) && !isInWake(index, e))
				.findFirst()
				.orElse(null);
	}
	...
	public int[] move() {
		...
		while (!flag) {
			...
			int getRandomMovable = numMovPieces.get(randomMovable);

			PieceType p = movablePieces[getRandomMovable];
			int index = p.getValue();

			if (!Arrays.equals(xy[index], outArray)) {
				int[] currCell = xy[index];
				int[] nextMove = pieceActions.pieceMove(p);

				if (nextMove != null) {
					int[] nextCell = getMyMove(nextMove);

					if (isDifferent(currCell, nextCell)) {
						if (!isInWake(index, new Position(nextCell[0], nextCell[1]))) {
							addInWake(index, currCell, nextCell);
							flag = true;
							result = nextMove.clone();
						} else {
							Position newP = findFirstFreeCell(currCell, index);
							if (newP != null) {
								int dir = pieceActions.pathToDirection(newP, currCell);
								addInWake(index, currCell, newP.toArray());
								flag = true;
	...
	private void addInWake(int index, int[] currCell, int[] nextCell) {
	...
	private boolean isInWake(int index, Position p) {
		return buffer[index].contains(p);
	}

	private boolean isNull(Position p) {
		return board[p.row][p.col] == null;
	}

	private boolean isDifferent(int[] currCell, int[] nextCell) {
		return (currCell[0] != nextCell[0] && currCell[1] != nextCell[1])
				|| (currCell[0] == nextCell[0] && currCell[1] != nextCell[1])
				|| (currCell[0] != nextCell[0] && currCell[1] == nextCell[1]);
	}

	private int[] directionToPath(int[] move) {
		int cordX = move[0], cordY = move[1];
		int dir = move[2], steps = move[3];
		if (dir == UP) {
	...
	private int posPieceValue(int[] current) {
		return IntStream.range(0, xy.length)
				.filter(e -> Arrays.equals(xy[e], current))
				.findFirst()
				.getAsInt();
	}
